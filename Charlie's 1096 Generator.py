...
from PyQt5.QtGui import QFont, QDesktopServices, QPainter, QPen
...
import html_writer
from open_excel import create_excel
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Charlie's 1096 Generator")
        ...
        quicken_file_button = QPushButton("Add Quicken File")
    ...
    def on_get_data_clicked(self):
        ...
        # get_data(self.selected_file_path, self.selected_company)
        qif_path = get_data(self.selected_file_path, self.selected_company)
        # print(qif_path)
        self.subs_df = subs_report.subs_df(qif_path)

        # Create and open the 2022 subcontractor report for the selected company        
        self.excel_file = create_excel(self.selected_company, self.subs_df)
        ...
        for column in selected_row:
            ...
            if pd.isna(column_value) or column_value == "":
                ...
                line_edit.setStyleSheet("border: 1px solid red;")  # Set red outline for empty fields
            else:
        ...
        # Create Open Subcontractor Report button
        subcontractor_report_button = QPushButton("Open Subcontractor Report")
        subcontractor_report_button.clicked.connect(self.open_subcontractor_report)
        layout.addWidget(subcontractor_report_button)
    ...
    def on_generate_forms_clicked(self):
        ...
        html_path = html_writer.populate_html(self.selected_company, self.excel_file)  # Get the path of the generated HTML file
        if html_path:
            QDesktopServices.openUrl(QUrl.fromLocalFile(html_path))

    def open_subcontractor_report(self):
        if self.excel_file:
